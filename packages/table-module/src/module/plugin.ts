...
function deleteCellBreak(
  ...
  if (!Text.isText(targetNode[0]) || targetNode[0].text.length < 2) {
    return false
  } // 如果存在\n\r，那长度必定大于2

  // 处理光标在换行符首/尾的情况,|表示光标  |\n\r   \n\r|
  const parameters: Parameters<typeof String.prototype.slice> =
    direction === 'backward'
  ...
  if (isBreak) {
    Transforms.insertText(
      newEditor,
      nodeText.slice(0, parameters[0]) + nodeText.slice(parameters[1]),
      {
        at: {
  ...
  if (isHalfBreak(newEditor, targetPoint)) {
    Transforms.insertText(
      newEditor,
      nodeText.slice(0, selection.anchor.offset - 1) + nodeText.slice(selection.anchor.offset + 1),
      {
        at: {
          anchor: Editor.start(newEditor, targetPoint.path),
          focus: Editor.end(newEditor, targetPoint.path),
  ...
  newEditor.deleteBackward = unit => {
    ...
    if (selection) {
      ...
      if (before) {
        ...
        if (
          !tableCell &&
          isTableOnBeforeLocation &&
          DomEditor.getSelectedNodeByType(newEditor, 'paragraph')
        ) {
          return
        }
      }
